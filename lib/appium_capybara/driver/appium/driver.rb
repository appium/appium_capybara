module Capybara::Appium
  class Capybara::Appium::Driver < Capybara::Driver::Base
    DEFAULT_OPTIONS = {
    }
    ...
    def find_xpath(selector)
      browser.find_elements(:xpath, selector).map { |node| Capybara::Appium::Node.new(self, node) }
    end

    def find_css(selector)
      browser.find_elements(:css, selector).map { |node| Capybara::Appium::Node.new(self, node) }
    end

    def find_custom(finder, locator)
      browser.find_elements(finder, locator).map { |node| Capybara::Appium::Node.new(self, node) }
    end
